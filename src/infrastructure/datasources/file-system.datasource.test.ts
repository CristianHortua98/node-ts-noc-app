...
describe('FileSystemDatasource', () => {
    ...
    test('should throw an error if severity level is not defined', async () => {
        ...
        const cumtomSeverityLevel = 'SUPER_HIGH' as LogSeverityLevel;

        try{

            await logDatasource.getLogs(cumtomSeverityLevel);
            expect(true).toBe(false);

        }catch(error){

            const errorString = `${error}`;
            expect(errorString).toContain(`${cumtomSeverityLevel} not implemented`);
